...
There is NO warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
  ...
  with ToolchainProfiler.profile_block('parse arguments and setup'):
    ...
    # If we are building side module then the wasm target is final output.
  ...
  try:
    ...
    if final_suffix in ('.o', '.bc', '.so', '.dylib') and not shared.Settings.SIDE_MODULE:
     diagnostics.warning('emcc', 'generated as exectuable with an object extension (%s).  If you meant to build an object file please use `-c, `-r`, or `-shared`' % final_suffix)
